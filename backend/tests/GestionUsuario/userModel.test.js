...
describe('userModel', () => {
  ...
  describe('additional integration tests', () => {
    ...
    test('updateUser actualiza campos', async () => {
      pool.query.mockResolvedValueOnce({
        rows: [{ id: 1, username: 'newu', created_at: 't', updated_at: 't' }],
      });
      const out = await userModel.updateUser(1, 'newu');
      expect(out?.username).toBe('newu');
    });

    test('updateUser lanza si no hay campos', async () => {
      await expect(userModel.updateUser(1, undefined)).rejects.toThrow('Falta el campo username');
    });
